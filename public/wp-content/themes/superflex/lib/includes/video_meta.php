	...
	class videoMeta {
		...
		var $videometa =	array(
			...
			array(
				...
				"description"	=> "Write the URL of the video- or swf-file here. <br/>For Vimeo, write the url to the page holding the video. (Example: http://vimeo.com/12358381)<br/><br/> For Youtube, write the same url as used for sharing movies (Example: http://www.youtube.com/watch?v=LpvIfXtA-v0). ",
				"type"			=> "text",
				"scope"			=>	array( "post","page","portfolio","product","news" ),
			...
			array(
				...
				"name"			=> "description",
				"title"			=> "Video/flash height",
				"description"	=> "Note: The width and height settings are not need when adding Vimeo or Youtube videos, as the default settings are compatible with these video's aspect ratios.",
				"type"			=> "html",
				"scope"			=>	array( "post","page","portfolio","product","news" ),
				"capability"	=> "edit_pages"
			),
		...
		*/
		function createCustomFields() {
			if ( function_exists( 'add_meta_box' ) ) {
				add_meta_box( 'my-custom-fields', 'Video or Flash content', array( &$this, 'displayCustomFields' ), 'phinews', 'normal', 'high' );
				add_meta_box( 'my-custom-fields', 'Video or Flash content', array( &$this, 'displayCustomFields' ), 'phiproduct', 'normal', 'low' );
				add_meta_box( 'my-custom-fields', 'Video or Flash content', array( &$this, 'displayCustomFields' ), 'phiportfolio', 'normal', 'high' );
				add_meta_box( 'my-custom-fields', 'Video or Flash content', array( &$this, 'displayCustomFields' ), 'phitestimonial', 'normal', 'high' );
				...
				foreach ( $this->videometa as $customField ) {
					...
					foreach ( $scope as $scopeItem ) {
						switch ( $scopeItem ) {
							...
							case "news": {
								// Output on any post screen
								if ( basename( $_SERVER['SCRIPT_FILENAME'] )=="post-new.php" || $post->post_type=="phinews" )
									$output = true;
								break;
							}
							case "product": {
								// Output on any post screen
								if ( basename( $_SERVER['SCRIPT_FILENAME'] )=="post-new.php" || $post->post_type=="phiproduct" )
									$output = true;
								break;
							}
							case "testimonial": {
								// Output on any post screen
								if ( basename( $_SERVER['SCRIPT_FILENAME'] )=="post-new.php" || $post->post_type=="phitestimonial" )
									$output = true;
								break;
							}
							case "portfolio": {
