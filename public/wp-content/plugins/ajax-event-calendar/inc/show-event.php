<?php
	...
	if (!empty($event->linkCalendar)) {
		if ($adjust < 1) {
			$adjust = 1;
		}
		$out .= str_repeat("<br>", $adjust);
		$linkCalendar  = "<a href='{$event->linkCalendar}' class='link round5 link-calendar cat{$event->category_id}' target='_blank'>";
		$linkCalendar .= "<i class='icon-insert-invitation'> </i>";
		$linkCalendar .= __('Adicionar ao meu calendário', AEC_NAME);
		$linkCalendar .= "</a>\n";
		$out .= "{$linkCalendar}";
	}
