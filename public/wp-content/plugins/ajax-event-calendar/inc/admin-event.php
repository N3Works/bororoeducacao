<?php
	...
	if (isset($_POST['event'])) {
		...
		if (isset($_POST['event']['id'])) {
			...
			$event->linkCalendar 	= $this->render_i18n_data($event->linkCalendar);
...
<body>
	...
	<ul>
		...
		<?php
			...
			if ($aec_options['linkCalendar'] > 0) { 
				
				$newStartDate = date("Ymd", strtotime($event->start_date) );
				$newEndDate = date("Ymd", strtotime($event->end_date.'+1 day') );

				$strLinkCalendar .= "https://www.google.com/calendar/render";
				$strLinkCalendar .= "?action=TEMPLATE";
				$strLinkCalendar .= "&text=".$event->title;
				//$strLinkCalendar .= "&dates=[start-custom format='Ymd\\THi00\\Z']/[end-custom format='Ymd\\THi00\\Z']";
				$strLinkCalendar .= "&dates=".$newStartDate."/".$newEndDate;
				$strLinkCalendar .= "&details=".$event->description;;
				$strLinkCalendar .= "&location=";
				$strLinkCalendar .= "&sf=true";
				$strLinkCalendar .= "&output=xml";

		?>
        <li style="display: none;">
			<label for="linkCalendar"><?php _e('Link Calendar', AEC_NAME); ?></label>
            <input type="text" name="linkCalendar" id="linkCalendar" class="wide" value="<?php echo $strLinkCalendar; ?>">
		</li>
		<?php
			}
